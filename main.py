...
def user():
    ...
    sql = "SELECT id user_id,nickname,avatar,lng AS `long`,lat,DATE_FORMAT(created_at,'%%Y-%%m-%%d %%H:%%m:%%s') login_time FROM users WHERE created_at > DATE_sub(NOW(), INTERVAL {} SECOND) AND lng IS NOT NULL".format(
        interval)
