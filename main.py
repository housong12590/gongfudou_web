...
def total_count():
    ...
    users = SQLHelper.fetch_all(
        "SELECT id user_id,nickname,avatar,lng ,lat ,unix_timestamp(created_at) login_time FROM users WHERE to_days(created_at) = to_days(now()) AND `lng` IS NOT NULL ORDER BY id DESC LIMIT 50")
